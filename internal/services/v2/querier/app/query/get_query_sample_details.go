...
func (h *GetQuerySampleDetailsHandler) Handle(ctx context.Context, snapID string, sampleID string) (*querierv2.GetSampleDetailsResponse, error) {
	...
	participants := make([]*querierv2.BlockChain_BlockingNode, len(roots))
	traveled := make(map[string]struct{}, 0)
	for i, root := range roots {
		participants[i] = h.buildblockNode(sampleMap, root, traveled)
	}
	planFound := true
	plan, err := h.repo.GetExecutionPlan(ctx, baseQuery.PlanHandle, snap.SnapInfo.Server)
	if err != nil {
	...
	return &querierv2.GetSampleDetailsResponse{
		QuerySample: converters.SampleToProto(baseQuery),
		ParsedPlan:  protoParsedPlan,
		BlockChain:  &querierv2.BlockChain{Roots: participants},
	}, nil
...
func (h *GetQuerySampleDetailsHandler) buildblockNode(sampleMap map[string][]*common_domain.QuerySample, sessionID string, traveled map[string]struct{}) *querierv2.BlockChain_BlockingNode {
	...
	childNodes := make([]*querierv2.BlockChain_BlockingNode, 0, len(sample.Block.BlockedSessions))
	...
	return &querierv2.BlockChain_BlockingNode{
