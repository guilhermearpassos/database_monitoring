...
const waitEventToIntensity: Record<string, number> = {
    ...
    "LATCH_EX": 0.5,
    "LATCH_SH": 0.55,
    "PREEMPTIVE_OS_FLUSHFILEBUFFERS": 0.6,
    "PREEMPTIVE_OLEDBOPS": 0.65,
    "WAIT_ON_SYNC_STATISTICS_REFRESH":0.7,
};
const waitEventToGroup: Record<string, string> = {
    ...
    "LATCH_EX": "Miscellaneous",
    "LATCH_SH": "Miscellaneous",
    "PREEMPTIVE_OS_FLUSHFILEBUFFERS": "Miscellaneous",
    "PREEMPTIVE_OLEDBOPS": "Miscellaneous",
    "WAIT_ON_SYNC_STATISTICS_REFRESH": "Miscellaneous",
};

export function getWaitEventColor(waitEvent: string): string {
    // Get the group for the wait event
    const color = getWaitEventColorNoDefault(waitEvent);
    if (color ==="" || color == null) {
        return "107, 114, 128, 1"
    }
    return color
}
export function getWaitEventColorNoDefault(waitEvent: string): string {
    // Get the group for the wait event
